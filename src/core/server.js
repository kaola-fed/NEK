...
const { js_beautify, html } = require('js-beautify');
...
class Server {
    ...
    async getMeta() {
        ...
        try {
            ...
            const { data } = await request(`${rc.serverApi}/api/public/page?id=${this.key}`, { json: true });
            this.meta = {
                url: data.url,
                type: data.type,
                ftl: data.ftl,
    ...
    format(content, ext) {
        let out = content;
        if (ext === 'js') {
            out = js_beautify(content, { indent_size: 4 });
